public class BSTScore {
    ...
    private int size(Node nodo) {
        ...
        int izqSize = size(nodo.getLeft());
        int derSize = size(nodo.getRight());
        return 1 + izqSize + derSize;
    }
    ...
    private void reverseInOrder(Node current){
        if(current == null){
            return;
        }
        reverseInOrder(current.getRight()); // primero visitamos el subárbol derecho
        System.out.print("Score: "+current.getValue()+", Player: "+current.getNamePlayer()+"\n");
        reverseInOrder(current.getLeft()); // luego el subárbol izquierdo
    }
    ...
    private Node delete(int goal, Node current){
        ...
        if(current.getValue() == goal){
            ...
            //4. Nodo con dos hijos
                ...
                current.setValue((int) min.getValue());
                //Hacer eliminación a partir de la derecha
                Node subarbolDER = delete((int) min.getValue(), current.getRight());
